...
DATA_DIR                = os.getcwd()+'/../data/test_gain_tuning/'
...
Q_pos   = matlib.diagflat(np.matrix([1.,1.,0.] + ny*[0.] + nf*[0.] + nf*[0.] + nf*[0.0]))
Q_ddf   = matlib.diagflat(np.matrix(ny*[0.0] + ny*[0.] + nf*[0.] + nf*[0.] + nf*[1.]))
expected_cost_pos = {}
expected_cost_ddf = {}
...
if(not LOAD_DATA):
    ...
    for (ctrl, f_dist, zeta, w_ddf) in itertools.product(controllers, f_dists, zetas, w_ddf_list):
        test_name = ctrl + '_zeta_'+str(zeta)+'_fDist_'+str(f_dist)+'_w_ddf_'+str(w_ddf)
        ...
        if(True):
            ...
            for i in range(ny):
                ...
                ax[i].set_title('CoM '+str(i)+', w_ddf='+str(w_ddf))
                ax[i].legend()
...
#            print 'real MSE com   %.6f'%data.com_pos_mse
            ...
            for i in range(nf):
                ...
                ax[i].set_title('f '+str(i)+', w_ddf='+str(w_ddf))
                ax[i].legend()
            ...
            for i in range(nf):
                ...
                ax[i].set_title('df '+str(i)+', w_ddf='+str(w_ddf))
                ax[i].legend()
            ...
            for i in range(nf):
                ...
                ax[i].set_title('ddf '+str(i)+', w_ddf='+str(w_ddf))
                ax[i].legend()
...
for (w_ddf, color) in zip(optimal_gains.keys(), colors):
    plt.plot(expected_cost_pos[w_ddf], expected_cost_ddf[w_ddf], ' *', color=color, markersize=30, label='expected, w_ddf='+str(w_ddf))
